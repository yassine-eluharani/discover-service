...
from user_data import COVER_LETTER_DIR, get_resume_text, load_profile
from db import get_connection, upsert_user_job
from llm import get_client
from scoring.validator import (
    BANNED_WORDS,
    LLM_LEAK_PHRASES,
    sanitize_text,
...
def run_cover_letters(
    ...
    for job in jobs:
        ...
        try:
            ...
            # PDF generation is not done in the worker context — PDFs are
            # generated on-demand by the backend when the user clicks
            # download (scoring/pdf.py uses Playwright which we don't ship
            # here). Text-only is the contract.
            pdf_path = None
